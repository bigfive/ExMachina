defmodule Exmachina.Neuron do
  ...
  def new(num_inputs: num_inputs, output_pids: output_pids) do
    %Neuron{dendrites: Dendrites.new(num_inputs), axon: Axon.new(output_pids)}
  end
  ...
  def record_activation(%Neuron{dendrites: dendrites} = neuron, activity, from) do
    dendrites = Dendrites.add_input_activity(activity, from, dendrites)
    if map_size(dendrites.input_activities) == dendrites.num_inputs, do: GenServer.cast(self(), :fire)
